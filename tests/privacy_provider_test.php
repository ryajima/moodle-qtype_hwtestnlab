...
use qtype_hwtestnlab\privacy\provider;
...
global $CFG;
require_once($CFG->dirroot . '/question/type/hwtestnlab/classes/privacy/provider.php');
...
class qtype_hwtestnlab_privacy_provider_testcase extends \core_privacy\tests\provider_testcase {
    ...
    public function test_get_metadata() {
        $collection = new \core_privacy\local\metadata\collection('qtype_hwtestnlab');
        $actual = \qtype_hwtestnlab\privacy\provider::get_metadata($collection);
    ...
    public function test_export_user_preferences($name, $value, $expected) {
        $this->resetAfterTest();
        $user = $this->getDataGenerator()->create_user();
        set_user_preference("qtype_hwtestnlab_$name", $value, $user);
        provider::export_user_preferences($user->id);
        $writer = writer::with_context(\context_system::instance());
        $this->assertTrue($writer->has_any_data());
        $preferences = $writer->get_user_preferences('qtype_hwtestnlab');
        foreach ($preferences as $key => $pref) {
            $preference = get_user_preferences("qtype_hwtestnlab_{$key}", null, $user->id);
            if ($preference === null) {
                continue;
            }
            $desc = get_string("privacy:preference:{$key}", 'qtype_hwtestnlab');
