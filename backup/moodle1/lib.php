...
class moodle1_qtype_hwtestnlab_handler extends moodle1_qtype_shortanswer_handler {
    /**
     * @return array
     */
    ...
    public function process_question(array $data, array $raw) {
        ...
        if (isset($data['hwtestnlab'][0]['wiriseditor'])) {
            $wiriseditor = array();
            parse_str($data['hwtestnlab'][0]['wiriseditor'], $wiriseditor);
        ...
        $data['actualhwtestnlab']['id'] = $this->converter->get_nextid();

        $wirisprogram = '<question><wirisCasSession>';
        $wirisprogram .= htmlspecialchars(wrsqz_mathml_decode($data['hwtestnlab'][0]['wirisprogram']), ENT_COMPAT, "utf-8");
        ...
        $data['actualhwtestnlab']['xml'] = $wirisprogram;
        $this->write_xml('question_xml', $data['actualhwtestnlab'], array('/question_xml/id'));
    }
    ...
    protected function wrsqz_get_cas_for_computations($data) {
        ...
        $wirisquestion = '';
        if (isset($data['hwtestnlab'][0]['wirisoptions'][0]['wiriscasforcomputations'])) {
            if ($data['hwtestnlab'][0]['wirisoptions'][0]['wiriscasforcomputations'] == 1) {
                ...
                $wirisquestion .= '</data>';
            } else if ($data['hwtestnlab'][0]['wirisoptions'][0]['wiriscasforcomputations'] == 2) {
                // @codingStandardsIgnoreStart
                $wirisquestion .= '<data name="' . com_wiris_quizzes_api_QuizzesConstants::$PROPERTY_SHOW_CAS . '">';
    ...
    protected function wrsqz_hidden_initial_cas_value($data) {
        ...
        $wirisquestion = '';
        if (isset($data['hwtestnlab'][0]['wirisoptions'][0]['hiddeninitialcasvalue'])) {
            // @codingStandardsIgnoreLine
            $wirisquestion .= '<data name="' . com_wiris_quizzes_api_QuizzesConstants::$PROPERTY_CAS_INITIAL_SESSION . '">';
            $initialcasvalue = $data['hwtestnlab'][0]['wirisoptions'][0]['hiddeninitialcasvalue'];
    ...
    protected function wrsqz_get_extra_parameters($data, $iscompound, $originalanswertext) {
        ...
        if (count($wiriseditor) > 0) {
            ...
            if (isset($data['hwtestnlab'][0]['wirisoptions']) && count($data['hwtestnlab'][0]['wirisoptions']) > 0) {
